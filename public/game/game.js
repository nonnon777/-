...
function SendData() {
    ...
    let phpurl = "http://www.gesipepa-cycle.com/game/score.php";
...
//終了
var block_pattern_next = [[0, 0, 0], [0, 0, 0], [0, 0, 0]];
var block_pattern = [[0, 0, 0], [0, 0, 0], [0, 0, 0]];
const p0 = [[0, 1, 0],[1, 1, 1], [ 0, 0, 0]];   //凸
const p1 = [[0, 1, 0], [0, 1, 0], [0, 1, 0]];    //｜
const p2 = [[1, 1, 0], [0, 1, 1], [0, 0, 0]];   //N
const p3 = [[0, 1, 1], [1, 1, 0], [0, 0, 0]];   //Nの逆
const p4 = [[0, 1, 0], [0, 1, 0], [0, 1, 1]];   //L
const p5 = [[0, 1, 0], [0, 1, 0], [1, 1, 0]];   //Lの逆
const p6 = [[0, 0, 0], [0, 1, 0], [0, 0, 0]];   // ・
const p7 = [[1, 1, 0], [1, 1, 0], [0, 0, 0]];   //四角
const p8 = [[1, 0, 0], [0, 1, 0], [0, 0, 1]];   //斜め
const p9 = [[0, 0, 0], [1, 1, 1], [1, 0, 1]];   //U
const p10 = [[0, 1, 0], [1, 1, 1], [0, 1, 0]];   //+

const p11 = [[0, 1, 0], [0, 1, 0], [0, 1, 0]];    //｜
const p12 = [[0, 1, 0], [0, 1, 0], [0, 1, 0]];    //｜
const p13 = [[0, 1, 0], [1, 1, 1], [0, 0, 0]];   //凸
const p14 = [[0, 0, 0], [0, 1, 0], [0, 0, 0]];   // ・
const block_pattern_memory = [p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14]
random();
random();
setcolor();
function keypress_ivent(e) {    //キーイベント　押したとき
...
function keyup_ivent(e) {    //キーイベント 離した時
...
function main(){
    ...
    if (start == 1) {
        ...
        if(falltest() == 1){
            ...
            setblock();
            block_pattern = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]; 
            setcolor();
            line();
            overtest();                      
            random();
...
function setcolor() { //blockの値に応じて色の変更
    for (let i = 0; i < 10; i++) {
        for (let j = 0; j < 20; j++) {
            if (block[i][j] == 1) {
...
function patterncolor(){  //block_patternに応じて色の変更
    for (let i = 0; i < 3; i++){
        for (let j = 0; j < 3; j++){
            let ii = i + x;
            let jj = j + y;
...
function setblock(){   //block_patternの値をblockcoloerに代入
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            if (block_pattern[i][j] == 1) {
                block[x + i][y + j] = 1;
...
function falltest(){    //下のblock検知 あるなら0,ないなら1
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            if(block_pattern[i][j] == 1){
...
function lefttest() {   //左のブロック検知 あるなら0,ないなら1
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            if(block_pattern[i][j] == 1){
...
function righttest() {  //右のブロック検知 あるなら0,ないなら1
    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            if(block_pattern[i][j] == 1){
                if (block[x + i + 1][y + j] == 1) {
...
function random(){      //ランダムpatternとnext_block表示
    var min = 0;
    var max = 14;
...
function gamereset(){
    ...
    var text = document.getElementById("scoredisp");
    text.textContent = "score:" + score;
    block_pattern_next = [[0, 0, 0], [0, 0, 0], [0, 0, 0]];
    block_pattern = [[0, 0, 0], [0, 0, 0], [0, 0, 0]];
    random();
    random();
    setcolor();
